 ...
 */
public class RemoveCoinsCommand
        implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        if(!(sender instanceof Player)) return true;
        Player player = (Player) sender;
        if(!(player.hasPermission("coinsapi.removecoins")) || !(player.hasPermission("coinsapi.administrator"))) {
        ...
        if(args.length != 2) {
            player.sendMessage(CoinsAPI.getInstance().getPrefix() + CoinsAPI.getInstance().getConfig().getString("Messages.commandSyntaxHelp")
                    .replace("&", "§").replace("%commandSyntax%", "/removecoins <player> <amount>"));
            return true;
        }
        ...
        try {
            ...
            if(CoinsProvider.isPlayerInDatabase(player1)) {
                CoinsProvider.removeCoins(player1, amount);
                player.sendMessage(CoinsAPI.getInstance().getPrefix() + CoinsAPI.getInstance().getConfig().getString("Messages.removePlayerCoins")
